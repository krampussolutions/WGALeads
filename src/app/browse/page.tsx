  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-6xl px-6 py-12">
        ...
        <p className="mt-2 text-slate-300">Contractors and realtors serving West Georgia.</p>

        <form method="get" className="mt-6 grid gap-3 rounded-xl border border-slate-800 bg-slate-900/40 p-4 md:grid-cols-5">
          <input name="q" defaultValue={q} placeholder="Search…" className="rounded-md border border-slate-700 bg-slate-950 px-3 py-2" />
          <input name="category" defaultValue={category} placeholder="Category" className="rounded-md border border-slate-700 bg-slate-950 px-3 py-2" />
          <input name="city" defaultValue={city} placeholder="City" className="rounded-md border border-slate-700 bg-slate-950 px-3 py-2" />
          <input name="county" defaultValue={county} placeholder="County" className="rounded-md border border-slate-700 bg-slate-950 px-3 py-2" />
          <select name="type" defaultValue={type} className="rounded-md border border-slate-700 bg-slate-950 px-3 py-2">
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <div className="truncate text-lg font-semibold text-white">{l.business_name}</div>
                        {l.is_featured ? <span className="rounded-md border border-slate-700 px-2 py-0.5 text-xs text-slate-300">Featured</span> : null}
                      </div>
                      <div className="mt-1 text-sm text-slate-300">{l.category}{l.account_type ? ` · ${l.account_type}` : ""}</div>
                      <div className="mt-1 text-sm text-slate-400">
                        {l.city}, {l.state}{l.county ? ` · ${l.county} County` : ""} · {l.service_area}
                      </div>
