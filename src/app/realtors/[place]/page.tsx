...
function parsePlace(place: string) {
  ...
  const state = lower.endsWith("-ga") ? "GA" : "GA";
  if (isCounty) {
    const name = lower.replace("-county-ga", "").replaceAll("-", " ");
    return { kind: "county" as const, name: unslug(name), state };
  }
  const name = lower.replace("-ga", "").replaceAll("-", " ");
  return { kind: "city" as const, name: unslug(name), state };
}

export default async function RealtorsPlacePage({ params }: { params: { place: string } }) {
    ...
    <>
      ...
      <main className="mx-auto max-w-6xl px-6 py-12">
        <h1 className="text-3xl font-semibold">
          Realtors in {p.name}, {p.state}
        </h1>
        <p className="mt-2 text-slate-300">
          Browse local realtors. Request a quote or consultation from any listing.
        </p>
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          ...
          {(!listings || listings.length === 0) && (
            <Card>
              <p className="text-slate-300">No realtor listings found for this area yet.</p>
            </Card>
          )}
        </div>
      </main>
